async function drawGraph(
  ...
  let { index, links, content } = await fetchData
  ...
  const parseIdsFromNodes = (nodeEntries) => [
    ...new Set(nodeEntries.flatMap((nodeEntry) => [nodeEntry[0]])),
  ]
  ...
  let orphanPages = []
  if (displayOrphans) {
    const preOrphanPages = Object.entries(content).filter(
      (c) =>
        c[0] === "/" ||
        (!links.find((l) => l.target.includes(c[0])) &&
          !links.find((l) => l.source.includes(c[0]))),
    )
    // console.log(preOrphanPages)
    parseIdsFromNodes(preOrphanPages).forEach((id) => orphanPages.push(id))
    // console.log(orphanPages)
  }

  const data = {
    nodes: [...neighbours, ...orphanPages].map((id) => ({ id })),
